...
//! 脚手架项目工作区。引用已有的共享 system KB 与 compliance KB。
...
pub fn init(name: &str, system: &str, framework: &str, level: Option<u8>) -> eyre::Result<()> {
    ...
    if dir.exists()
        && fs::read_dir(dir)
            .wrap_err_with(|| format!("读取项目目标目录 {} 失败", dir.display()))?
            .next()
            .is_some()
    {
        eyre::bail!("项目目标目录非空: {}", dir.display());
    }
    ...
    if index.controls.is_empty() {
        eyre::bail!("框架 {framework} 没有控制项，不能初始化评估项目");
    }
    ...
    if control_count == 0 {
        eyre::bail!("框架 {framework} 在所选级别下没有适用控制项");
    }

    // 项目必须引用显式初始化的系统，避免静默创建 display_name=slug 的半成品 KB。
    let system_index = system_dir(system)
    ...
    crate::storage::transaction(|| {
        crate::storage::create_dir_all(&dir.join("facts")).wrap_err("创建 facts 目录失败")?;
        crate::storage::create_dir_all(&dir.join("evidence")).wrap_err("创建 evidence 目录失败")?;
        crate::storage::create_dir_all(&dir.join("drafts")).wrap_err("创建 drafts 目录失败")?;
        crate::storage::atomic_write(&dir.join("project.yaml"), &meta_yaml)
            .wrap_err("写 project.yaml 失败")?;
        crate::storage::atomic_write(&dir.join("facts/index.yaml"), "facts: []\n")
            .wrap_err("写 facts/index.yaml 失败")?;
        crate::storage::atomic_write(&dir.join("evidence.yaml"), "evidence: {}\n")
            .wrap_err("写 evidence.yaml 失败")?;
        crate::storage::atomic_write(&dir.join("matrix.yaml"), &matrix_yaml)
            .wrap_err("写 matrix.yaml 失败")
    })
    .wrap_err("初始化项目存储事务失败")?;
